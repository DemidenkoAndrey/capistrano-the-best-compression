namespace :deploy do
  ...
  task :compress_assets, :names do |_, args|
    on roles(:app) do
      ...
      execute "find -L #{assets_path + 'assets' } \\( -name \"#{names_args}\" \\) -exec bash -c \"[ ! -f '{}.gz' ] && zopfli --gzip --i20 '{}'\" \\; "
    end
  end
  ...
  task :compress_png do
    on roles(:app) do
      assets_path = release_path.join('public')
      execute "find -L #{assets_path + 'assets'} \\( -name *.png \\) -not \\( -name 'zopflied_*.png' \\) -exec bash -c 'FULLPATH='{}'; FILENAME=${FULLPATH##*/}; BASEDIRECTORY=${FULLPATH%$FILENAME}; [ ! -f \"${BASEDIRECTORY}zopflied_${FILENAME}\" ] && zopflipng \"${FULLPATH}\" \"${BASEDIRECTORY}zopflied_${FILENAME}\" ' \\; "
    end
  end
